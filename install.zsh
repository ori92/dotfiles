...
[ -n "$XDG_CONFIG_HOME" ] || {
    echo "${red}\$XDG_CONFIG_HOME is undefined, setting it to ~/.config"
...
if [[ $answer =~ ^[Yy]$ ]]; then
    ...
    echo "${green}->Creating backups..."
    [ -d "$XDG_CONFIG_HOME/zsh" ] && mv $XDG_CONFIG_HOME/zsh{,.bak} && echo "->$XDG_CONFIG_HOME/zsh.bak."
    [ -d "$HOME/.zshrc" ] && mv $HOME/.zshrc{,.bak} && echo "->$HOME/.zshrc.bak."
    [ -d "$HOME/.zshenv" ] && mv $HOME/.zshenv{,.bak} && echo "->$HOME/.zshenv.bak."
    [ -d "$XDG_CONFIG_HOME/zsh" ] && mv $XDG_CONFIG_HOME/zsh{,.bak} && echo "->$XDG_CONFIG_HOME/zsh.bak."
    [ -d "$XDG_CONFIG_HOME/zsh" ] && mv $XDG_CONFIG_HOME/zsh{,.bak} && echo "->$XDG_CONFIG_HOME/zsh.bak."
    [ -d "$XDG_CONFIG_HOME/tmux" ] && mv $XDG_CONFIG_HOME/tmux{,.bak} && echo "->$XDG_CONFIG_HOME/tmux.bak."
    [ -d "$XDG_CONFIG_HOME/starship.toml" ] && mv $XDG_CONFIG_HOME/starship.toml{,.bak} && echo "->$XDG_CONFIG_HOME/starship.toml"
    [ -d "$XDG_CONFIG_HOME/astronvim/" ] && mv $XDG_CONFIG_HOME/astronvim/{,.bak} && echo "->$XDG_CONFIG_HOME/astronvim.bak."
    echo "${green}->Done."

    read "answer?Install oh-my-zsh? [y/N]"
    if [[ $answer =~ ^[Yy]$ ]]; then
        echo "${green}Installing oh-my-zsh.."
        echo "${red}->Please exit zsh by typing 'exit' after omz installation is finished in order to complete this installation process."
    ...
    echo "${green}Installing zsh rc files.."
    ...
    echo "${green}Installing tmux rc files.."
    ...
    echo "${green}Installing tmux-plugin manager.."
    echo "${red}Remember to press prefix(C-b)+I to install plugins after you first run tmux."
    read "?Enter to continue.."
    git clone https://github.com/tmux-plugins/tpm "$XDG_CONFIG_HOME/tmux/plugins/tpm"

    echo "${green}Installing starship rc files.."
    ...
    if [[ $answer =~ ^[Yy]$ ]]; then
        ...
        echo "${red}Exit vim with :q after packer has finished to install the plugins.."
        read "?Enter to continue.."
        nvim +PackerSync
    fi
 
    echo "Installing astronvim rc files.."
